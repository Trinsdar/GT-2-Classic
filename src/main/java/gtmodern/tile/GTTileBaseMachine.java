...
public abstract class GTTileBaseMachine extends TileEntityElecMachine
	...
	public void process(GTMultiInputRecipeList.MultiRecipe recipe) {
	...
	public GTMultiInputRecipeList.MultiRecipe getRecipe() {
		...
		if (lastRecipe == null) {
			lastRecipe = getRecipeList().getPriorityRecipe(new Predicate<GTMultiInputRecipeList.MultiRecipe>() {

				@Override
				public boolean test(GTMultiInputRecipeList.MultiRecipe t) {
	...
	public boolean checkRecipe(GTMultiInputRecipeList.MultiRecipe entry, List<ItemStack> inputs) {
